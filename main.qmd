...
### Exercise 1.1: Sef-Play {.unnumbered}
...
I would like to have an interactive bandit problem.
Like 4 buttons and when you click them
you can see your reward.
...
## Introduction to importance sampling {.unnumbered}

Later for monte carlo methods we need importance sampling.
For me the idea is best introduced a bit earlier
in the multi-armed bandit scenario.

We have a policy $\pi$ for selecting
our action and want to know the expectation
of its value
$$v_\pi
= \mathbb{E}[r(A) \mid A \sim \pi]
= \sum_{a \in \mathcal{A}} r(a) \cdot \pi(a)
$$
We can get an estimate for $v_\pi$ by sampling
a bunch of times according to $\pi$
and get retunrs $R_1,\dots,R_N$ and estimate
$$v_\pi \approx \frac{\sum_i R_i}{N}$$

Now assume we are forced to follow a different
behaviour policy $b$ but still want to obtain 
an estimate for $v_\pi$.
If we sample according to $b$ and take the mean
we get an estimate for $v_b$.
But we can turn those samples into an estimate for $v_\pi$.
We can express $v_\pi$ as
$$
\begin{align*}
...
\end{align*}
$$
So we can sample according to $b$ list the choosen actions 
$A_1, \dots, A_n$ and the reward $R_1, \dots, R_n$
and estimate
$$
...
We have this litte calculation:
$$
\begin{split}
...
is not applicable here because we are sampling according to $b$
...
How can I derive the the correct formula
$$
V(S) \gets V(S) + \alpha  \frac{\pi(A)}{b(A)}[ R + \gamma V(S') - V(S)]
$$
then?
...
So in normal sampling we would have take samples in state $S$
$$
v_\pi(S) \approx \frac{\sum_{t \in \mathcal{T}(s)} \rho_{t:t} [R_t + \gamma v_\pi(S_{t+1})]}{| results |}
$$

So in the offline algorithm we have to shift our estimate $V(S)$ towards
$\frac{\pi(A)}{b(A)} [ R + \gamma V(S')]$.
In detail this means when we are in state $S$ and $b$ takes action $A$
with reward $R$ and resulting state $S'$ we have to move $V(S)$
more towards $\frac{\pi(A)}{b(A)}[R + \gamma V(S')]$
...
\for{for each episode}
    ...
    \repeat
        \state $A \gets \pi(S)$
        \state take action $A$ according to $b$, observe $R,S'$
        \state $V(S) \gets V(S) + \alpha \frac{\pi(A)}{b(A)} [ R + \gamma V(S') - V(S)]$
        \state $S \gets S'$
    \until{S is terminal}
\endfor 
\end{algorithmic}
