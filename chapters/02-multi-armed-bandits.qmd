...
The time index here doesn’t play a special role as the action-reward probabilities in the armed bandit are stationary. You can think of the condition meaning "when $a$ is picked".
...
$\Omega$ is the set of all possible trajectories on the environment, i.e., complete sequences of outcomes for the random variables describing a run of the process, and $\mathrm{Pr}$ assigns a probability to each trajectory:
$$
\mathrm{Pr}\colon \Omega \to [0,1] \quad \text{with} \quad
\sum_{\omega \in \Omega} \mathrm{Pr}(\omega) = 1.
$$

*Random variables* are simply functions $X\colon \Omega \to \mathcal{X}$ from $\Omega$ to a result space $\mathcal{X}$.
...
So an arbitrary but concrete trajectory looks like this
$A_1(\omega), R_1(\omega), A_2(\omega), R_2(\omega) \dots$

Let's bring up two common conventions are used for this notation $\mathbb{E}[ R_t \mid A_t = a]$ of the conditional expectation:

 1. We usually omit the argument $\omega$ when referring to the value of a random variable. This conflates the notation for $X$ the function and $X(\omega)$ a value of the function, but is easer to write than $\mathbb{E}[R_t(\omega) | A_t(\omega) = a]$.
 1. When dealing with functions over subsets of $\Omega$, we abbreviate expressions like $F({\omega \in \Omega : \text{statement true for }\omega})$ to simply $F(\text{statement})$. Without this we had to write $\mathbb{E}[R_t(\omega) | \omega \in \Omega : A_t(\omega) = a]$.
...
but that’s a technicality that won't come up.
...
A more commonly used form is the "law of the unconscious statistician"-theorem, also called LOTUS (see appendix @thm-law-of-the-unconscious-statistician):
$$
...
The 10-armed testbed will accompany us through the rest of this chapter. So here is the class that we use as the armed bandit environment.
...
And here's the code for the sample-average bandit algorithm. For clarity, I'll refer to this and upcoming algorithms as 'agents', given their learning behaviour. Note that this is using the incremental implementation from @sec-incremental-implementation.
...
[@sutton2018, Figure 2.2]. The performance of ε-greedy sample average agents on the 10-armed testbed for different values $\varepsilon$ averaged over 2000 runs.
::::

Out of curiosity, let’s see what happens when we have only one run, so there is no averaging to be done. It's a mess, wow. Without averaging over a couple of runs, we can't make out anything.
